...
from nutriciones.services.dieta_service import calcular_refeicao, salvar_plano_alimentar, PlanoAlimentar

def simular():
    meu_prato = [
        {"nome": "Arroz, integral, cozido", "peso_g": 150.0},
        {"nome": "Feijão, carioca, cozido", "peso_g": 100.0},
        {"nome": "Batata Asterix Frita de Fast Food", "peso_g": 120.0} # Deve printar aviso de não encontrado e abortar computo deste index
    ]
    ...
    for item in resultado["itens_analisados"]:
        ...
        print(f"    Kcal: {item['kcal']} | Prot: {item['proteina_g']}g | Lip: {item['lipidios_g']}g | Carb: {item['carboidratos_g']}g")
    ...
    print("==============================")
    print(f" Calorias:     {resultado['kcal']} kcal")
    print(f" Proteínas:    {resultado['proteina_g']} g")
    print(f" Lipídios:     {resultado['lipidios_g']} g")
    print(f" Carboidratos: {resultado['carboidratos_g']} g")
    print("==============================\n")
    ...
    novo_plano = PlanoAlimentar(
        ...
        itens_detalhados=json.dumps(resultado["itens_analisados"], ensure_ascii=False)
    )
